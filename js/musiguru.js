...
if(env.isProduction){
	...
	homeLibrary=["ZSe2z0P6bK","efED6kJjX0","iNZQVGUb1O","rLtxxgKQQ3","tqgKgCUtRD"];
}else{
...
/*
parse_app_id = "VCevSWMrfhAZHFKvZNOyf0BqRsW73rnHQdPsk7Iy";
parse_app_js_key =  "sLhr6iNDfd2z1tTdE28WoFAxxsYSNCP1VoTcThD6";
parse_app_rest_key = "0DUSmFNfS8So3fs2NG0KKDjpFyUPpCgC9jp61kZJ";
*/
...
function ajaxPageLoader(){
	$(".transparentcontainer").hide(0).delay(1500).fadeIn(0,function(){
...
function showPagePreLoader(className,speed){
	//alert(speed);
	if(className == "postlgn_library_loading_circle" ){
			//alert($(".verify_mail_section").is(":visible")) ;

	$(".post_login_section_content").hide(0).delay(speed).fadeIn(0,function(){

          $("."+className).hide();
    })

		//$("."+className).show(0).delay(speed).hide(0);
	}else{
		$("."+className).show(0).delay(speed).hide(0);	
	}
...
function lessonBoardUserNotification(){
	setTimeout(function() {
		addNotice('<p>Sign Up and Activate your account to access the videos and full features of the Musiguru.!</p>');
	}, 1000);
...
function addNotice(notice) {
	$('<div class="lesson_notify_notice"></div>')
		.append('<div class="lesson_notify_skin"></div>')
		.append('<a href="#" class="lesson_notify_close">close</a>')
		.append($('<div class="content"></div>').html(notice))
		.hide()
		.appendTo('#lesson_notify_growl')
		.fadeIn(1000).animate({
				bottom: "450px"
				//queue: false
			}, 1000, function() {
				$('#lesson_notify_growl .lesson_notify_close').on('click', function() {
					$('#lesson_notify_growl').remove();
					return false;	
				});
...
function getTrainingVideo(index, _isModuleLocked)
{
	...
	trackUserInteraction(window.location,"LessonBoard","LessonView", lessonsListObj[index].get("SKU_ID") +"==" + lessonsListObj[index].get("name"));
...
function renderCloudFrontVideo(m3u_url){
	...
	_url = _url.replace( /.mpg$/ , ".mp4");


	console.log("rtmp/'+_url====="+'rtmp/'+_url);
...
function appendOverlayDiv(){
	...
	window.setTimeout(function() { 
		$('#sub-content-two').prepend($('<div id=\'authuntication_overlay\'></div>'));
	    
	    if($("#authuntication_overlay").length > 0){
	    	var divHeight = $("#sub-content-two").height();	
	    	$("#authuntication_overlay").height(divHeight+15).css({ 'top': top+"px"});	
	    }
